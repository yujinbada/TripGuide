...
class SelectFestivalFragment : Fragment(), View.OnClickListener {
    ...
    private var mBinding: FragmentSelectFestivalBinding? = null
    ...
    override fun onCreateView(
        ...
        mBinding = FragmentSelectFestivalBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        Log.d(Constants.TAG, "SelectFestivalFragment - onViewCreated() called")
    ...
    private fun fetchXML2(url: String) {
        ...
        class getDangerGrade : AsyncTask<Void, Void, Void>() {
            ...
            override fun onPostExecute(result: Void?) {
                ...
                var tagProgram = false
                var tagEventstartdate = false
                var tagEventenddate = false
                var tagPlaytime = false
                var tagUsetimefestival = false
                var tagEventhomepage = false
                ...
                while (eventType != XmlPullParser.END_DOCUMENT) {
                    if (eventType != XmlPullParser.START_DOCUMENT) {
                        if (eventType == XmlPullParser.START_TAG) {
                            ...
                            if (tagName.equals("program")) tagProgram = true
                            else if (tagName.equals("eventstartdate")) tagEventstartdate = true
                            else if (tagName.equals("eventenddate")) tagEventenddate = true
                            else if (tagName.equals("playtime")) tagPlaytime = true
                            else if (tagName.equals("usetimefestival")) tagUsetimefestival = true
                            else if (tagName.equals("eventhomepage")) tagEventhomepage = true
                        }
                    }
                    if (eventType == XmlPullParser.TEXT) {
                        if (tagProgram) {
                            program = xpp.text.replace("<br />", "\n")
                            tagProgram = false
                            if(program != ""){
                                binding.seletedRoomtype.text = "행사 프로그램 : $program"
                            }
                            else {
                                binding.seletedRoomtype.text = "행사 프로그램 : 홈페이지 공지사항 참고"
                            }

                        }
                        if(tagEventstartdate) {
                            eventstartdate = xpp.text.replace("<br />", "\n")
                            tagEventstartdate = false
                            if(eventstartdate != null) {
                                binding.selectedCheckintime.text = "행사 시작일 : $eventstartdate"
                            }
                            else binding.selectedCheckintime.text = "행사 시작일 : 홈페이지 공지사항 참고"

                        }
                        if(tagEventenddate) {
                            eventenddate = xpp.text.replace("<br />", "\n")
                            tagEventenddate = false
                            if(eventenddate != null) {
                                binding.selectedCheckouttime.text = "행사 종료일 : $eventenddate"
                            }
                            else binding.selectedCheckouttime.text = "행사 종료일 : 홈페이지 공지사항 참고"

                        }
                        if(tagPlaytime) {
                            playtime = xpp.text.replace("<br />", "\n")
                            tagPlaytime = false
                            if(playtime != null) {
                                binding.selectedSubfacility.text = "공연 시간 : $playtime"
                            }
                            else binding.selectedSubfacility.text = "공연 시간 : 홈페이지 공지사항 참고"

                        }
                        if(tagUsetimefestival) {
                            usetimefestival = xpp.text.replace("<br />", "\n")
                            tagUsetimefestival = false
                            if(usetimefestival != null) {
                                binding.selectedReservationlodging.text = "이용 요금 : $usetimefestival"
                            }
                            else binding.selectedReservationlodging.text = "이용 요금 : 홈페이지 공지사항 참고"

                        }
                        if(tagEventhomepage) {
                            eventhomepage = xpp.text.replace("<br />", "\n")
                            tagEventhomepage = false
                            if(eventhomepage != null) {
                                binding.selectedRefundregulation.text = "행사 홈페이지 : $eventhomepage"
                            }
                            else binding.selectedRefundregulation.text = "행사 홈페이지 : 홈페이지 공지사항 참고"
    ...
    override fun onClick(v: View?) {
        when(v?.id) {
            R.id.back -> {
                mainActivity.removeFragment(SelectFestivalFragment())
            }
        }
    }
